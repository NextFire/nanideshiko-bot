...
class Greets(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.guildpics = json.loads(resources.read_binary(config.saves, 'greets.json'))

    @commands.Cog.listener()
    async def on_member_join(self, member):
        if member.guild.system_channel is not None:
            try:
                with resources.path(config.res, self.guildpics[str(member.guild.id)]) as path:
    ...
    def cog_unload(self):
        with resources.path(config.saves, 'greets.json') as path:
